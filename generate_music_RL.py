...
--transpose-to-C (transpose piano notes to C major / A minor, use only with --piano_only)
...
# --- EXAMPLE --------------------------------------------
# python dataset_creator.py --base_data_dir "C:\Users\Michael\Desktop\MusicDatasets\Datasets\adl_piano_midi" --transpose-to-C --piano_only --extract_genre --fast
# --------------------------------------------------------
...
# MODIFICA 1: Inizializza la variabile globale a None per garantirne l'esistenza in tutti i processi worker.
MIDI_TOKENIZER = None
...
def check_file_validity(args_tuple):
    ...
    """
    # MODIFICA: Accettiamo anche il flag is_fast_mode
    midi_file_path, transposition_enabled, is_fast_mode = args_tuple
    ...
    # MODIFICA: Esegui il controllo sulla chiave solo se la trasposizione E la modalità fast sono entrambe attive.
    # Se la modalità fast non è attiva, music21 analizzerà la chiave in un secondo momento.
    if transposition_enabled and is_fast_mode:
        declared_key = mido_check_result.get('key_signature_declared')
        if not declared_key:
            return None # Scarta se la trasposizione in modalità fast è richiesta ma la chiave non c'è
            
        if get_transposition_semitones(declared_key, config.REFERENCE_KEY_MAJOR, config.REFERENCE_KEY_MINOR) is None:
            return None # Scarta se la chiave non è nel nostro map per la modalità fast
...
def build_or_load_tokenizer_for_creator(midi_file_paths_for_vocab_build=None, force_build=False):
    ...
    if not VOCAB_PATH.exists() or force_build:
        ...
        # MODIFICA 2: Chiama tokenizer.train() solo se il tokenizer lo supporta (non per Octuple)
        is_trainable = not isinstance(tokenizer, (miditok.Octuple, miditok.CPWord, miditok.MuMIDI))
        ...
        if midi_file_paths_for_vocab_build:
            if is_trainable:
                ...
                logging.info(f"Il tokenizer {type(tokenizer).__name__} non richiede addestramento (es. Octuple). Salto questo passaggio.")
...
def save_octuple_vocab_sizes(tokenizer, path):
    """
    Per tokenizer Octuple, ispeziona i vocabolari componenti e salva le loro dimensioni.
    """
    if not isinstance(tokenizer, miditok.Octuple):
        logging.warning("Il tokenizer non è Octuple, salto il salvataggio delle dimensioni dei vocabolari componenti.")
        return

    # L'ordine è definito internamente in MidiTok per Octuple.
    # Dobbiamo rispettarlo: Pitch, Velocity, Duration, Program, Position, Bar, TimeSignature, Tempo
    # Controlliamo la documentazione o il codice sorgente di MidiTok per l'ordine esatto se necessario.
    # L'ordine standard è: Pitch, Velocity, Duration, Program, Position, Bar, TimeSig, Tempo
    vocab_names = tokenizer.vocab_types
    
    vocab_sizes = {name: len(vocab) for name, vocab in zip(vocab_names, tokenizer.vocab)}
    
    logging.info(f"Salvataggio delle dimensioni dei vocabolari Octuple in {path}")
    logging.info(f"Dimensioni rilevate: {vocab_sizes}")
    ...
    try:
        ...
        logging.error(f"Errore durante il salvataggio del file delle dimensioni del vocabolario {path}: {e}")
...
def init_worker(vocab_p, midi_tok_strat_class):
    """
    Funzione di inizializzazione per ogni processo worker del pool.
    Crea un'istanza del tokenizer per ogni processo.
    """
    global MIDI_TOKENIZER
    logging.info(f"Worker {os.getpid()} initializing tokenizer.")
    try:
        # Carica il tokenizer usando i parametri passati
        ...
        # Se fallisce, il tokenizer sarà None e il processo lo riporterà come errore
...
if __name__ == "__main__":
    ...
    # --- MODIFICA 3: Esegui il pre-filtraggio solo in modalità --fast ---
    if args.fast:
        logging.info("--- Fase 0: Pre-filtraggio dei file per la modalità --fast ---")
        valid_midi_files_for_processing = []
        pre_filter_tasks = [(path, args.transpose_enabled, args.fast) for path in all_potential_midi_files]
        ...
        logging.info("Modalità non-fast: salto il pre-filtraggio. Tutti i file verranno passati all'elaborazione principale.")
    ...
    try:
        # L'inizializzazione del tokenizer nel processo principale non è strettamente necessaria
        # per l'elaborazione dei worker, ma è utile per operazioni preliminari e per creare
        # il file di vocabolario la prima volta.
        main_process_tokenizer = build_or_load_tokenizer_for_creator(
            midi_file_paths_for_vocab_build=temp_midi_files_for_vocab,
            force_build=args.force_tokenizer_build
        )
        tokenizer_successfully_initialized = main_process_tokenizer is not None
        
        # Salva le dimensioni del vocabolario se si usa Octuple
        if tokenizer_successfully_initialized:
            # Controlla se il tokenizer è di tipo Octuple e, in caso, salva le dimensioni
            # dei suoi vocabolari componenti.
            if isinstance(main_process_tokenizer, miditok.Octuple):
                paths = config.get_project_paths(args.base_data_dir) # Assicurati che 'paths' sia definito
                MIDI_VOCAB_SIZES_PATH = paths.get("midi_vocab_sizes")
                if MIDI_VOCAB_SIZES_PATH:
                    save_octuple_vocab_sizes(main_process_tokenizer, MIDI_VOCAB_SIZES_PATH)
                else:
                    logging.error("Percorso 'midi_vocab_sizes' non trovato nella configurazione dei percorsi.")
        
    except Exception as e:
        logging.error(f"Impossibile inizializzare il tokenizer MIDI nel processo principale: {e}", exc_info=True)
